  ...
  if (interaction.isChatInputCommand()) {
    ...
    if(interaction.customId=="shop") {
      try {
        ...
        const userData = (await mod.getUser(interaction.guild.id, interaction.user.id)).data()
        ...
        for(var i in shopItems) {
          if(shopItems[i].data.label==selectedItem.values[0]) {
            ...
            if(userData.credits>=Number(shopItemsMod[i].cost)) {
              ...
              await giveCredits(mod,interaction,-shopItemsMod[i].cost,"Purchased "+shopItemsMod[i].name)
            }
            else {
              await selectedItem.reply({
